...
do
    ...
    function Funcs:AddLabel(...)
        ...
        if typeof(First) == "table" or typeof(Second) == "table" then
            ...
            Data.ColoredSegments = Params.ColoredSegments or {} -- New: colored text segments
        else
            Data.Text = First or ""
        ...
        local Label = {
            ...
            ColoredSegments = Data.ColoredSegments,
        ...
        -- Create main holder for rich text
        ...
        -- Function to create rich text
        local function CreateRichText()
            -- Clear existing children
            for _, child in pairs(LabelHolder:GetChildren()) do
                if child:IsA("TextLabel") then
                    child:Destroy()
                end
            end

            if #Label.ColoredSegments == 0 then
                -- Simple text label if no colored segments
                ...
                Label.MainTextLabel = TextLabel
            else
                -- Create layout for rich text
                local Layout = New("UIListLayout", {
                    FillDirection = Enum.FillDirection.Horizontal,
                    HorizontalAlignment = Groupbox.IsKeyTab and Enum.HorizontalAlignment.Center or Enum.HorizontalAlignment.Left,
                    VerticalAlignment = Enum.VerticalAlignment.Center,
                    Parent = LabelHolder,
                })

                -- Create text segments
                for i, segment in pairs(Label.ColoredSegments) do
                    ...
                    -- Set color after creation to avoid template conflicts
                    if segment.color then
                        SegmentLabel.TextColor3 = Color3.fromHex(segment.color)
                    else
                        -- Use default font color from theme
                        SegmentLabel.TextColor3 = Library.Scheme.FontColor
                        Library.Registry[SegmentLabel] = Library.Registry[SegmentLabel] or {}
                        Library.Registry[SegmentLabel].TextColor3 = "FontColor"
                    end
                end
            end
        end
        ...
        function Label:SetVisible(Visible: boolean)
            ...
            LabelHolder.Visible = Label.Visible
            Groupbox:Resize()
        end

        function Label:SetText(Text: string)
            Label.Text = Text
            if Label.MainTextLabel then
                Label.MainTextLabel.Text = Text
            end

            if Label.DoesWrap and Label.MainTextLabel then
                local _, Y = Library:GetTextBounds(Label.Text, Label.MainTextLabel.FontFace, Label.MainTextLabel.TextSize, Label.MainTextLabel.AbsoluteSize.X)
                LabelHolder.Size = UDim2.new(1, 0, 0, Y + 4 * Library.DPIScale)
            end

            Groupbox:Resize()
        end

        function Label:SetColoredSegments(segments)
            Label.ColoredSegments = segments
            CreateRichText()
            Groupbox:Resize()
        end
        ...
        if Label.DoesWrap and Label.MainTextLabel then
            local _, Y = Library:GetTextBounds(Label.Text, Label.MainTextLabel.FontFace, Label.MainTextLabel.TextSize, Label.MainTextLabel.AbsoluteSize.X)
            LabelHolder.Size = UDim2.new(1, 0, 0, Y + 4 * Library.DPIScale)
        elseif not Label.DoesWrap and #Label.ColoredSegments == 0 then
            New("UIListLayout", {
                FillDirection = Enum.FillDirection.Horizontal,
                HorizontalAlignment = Enum.HorizontalAlignment.Right,
                Padding = UDim.new(0, 6),
        ...
        if Data.DoesWrap and Label.MainTextLabel then
            local Last = LabelHolder.AbsoluteSize

            LabelHolder:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
                if LabelHolder.AbsoluteSize == Last then
                    return
                end
        ...
        Label.TextLabel = Label.MainTextLabel or LabelHolder
        Label.Container = Container
        if not Data.DoesWrap and #Label.ColoredSegments == 0 then
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        if WindowInfo.Icon then
            ...
            -- Separator line right after icon
            New("TextLabel", {
                BackgroundTransparency = 1,
        ...
        CurrentTabInfo = New("Frame", {
            Size = UDim2.fromScale(WindowInfo.DisableSearch and 1 or 0.7, 1),
            Visible = true,
            BackgroundTransparency = 1,
            Parent = RightWrapper,
        })
    ...
    function Window:AddTab(...)
        ...
        function Tab:AddGroupbox(Info)
            ...
            do
                ...
                ArrowIcon = New("ImageLabel", {
                    BackgroundTransparency = 1,
                    Image = Library:GetIcon("chevron-up").Url,
                    ImageColor3 = "FontColor",
                    ImageRectOffset = Library:GetIcon("chevron-up").ImageRectOffset,
                    ImageRectSize = Library:GetIcon("chevron-up").ImageRectSize,
                    ImageTransparency = 0.5,
                    Size = UDim2.fromScale(1, 1),
            ...
            function Groupbox:Collapse()
                ...
                TweenService:Create(ArrowIcon, Library.TweenInfo, {
                    Rotation = Groupbox.IsCollapsed and 180 or 0,
                }):Play()
            ...
            function Groupbox:Resize()
                if not Groupbox.IsCollapsed then
                    Background.Size = UDim2.new(1, 0, 0, GroupboxList.AbsoluteContentSize.Y + 53 * Library.DPIScale)
                end
            end
        ...
        function Tab:Show()
            ...
            if Description then
                ...
                if IsDefaultSearchbarSize then
                    SearchBox.Size = UDim2.fromScale(0.25, 1)
                end

                CurrentTabLabel.Text = Name
                CurrentTabDescription.Text = Description
            else
                -- Show tab name even without description
                CurrentTabInfo.Visible = true
                CurrentTabLabel.Text = Name
